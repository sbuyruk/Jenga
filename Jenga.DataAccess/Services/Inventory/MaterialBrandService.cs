    ...
    public class MaterialBrandService : IMaterialBrandService
    {
        ...
        public async Task<Result<List<MaterialBrand>>> GetAllAsync(CancellationToken cancellationToken = default)
        {
            ...
            catch (Exception ex)
            {
                ...
                return Result.Failure<List<MaterialBrand>>(Error.Unexpected("Marka listesi alinamadi.", ex, "MaterialBrand.GetAll.Failed"));
            }
        }
        ...
        public async Task<Result<MaterialBrand>> GetByIdAsync(int id, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                if (entity is null)
                    return Result.Failure<MaterialBrand>(Error.NotFound($"Marka bulunamadi (Id={id}).", "MaterialBrand.NotFound"));
        ...
        {
            if (brand == null)
                return Result.Failure(Error.Validation("Marka bos olamaz.", "MaterialBrand.Null"));
            try
            {
                await using var db = await _dbFactory.CreateDbContextAsync(cancellationToken);
        ...
        public async Task<Result> DeleteAsync(MaterialBrand brand, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (entity == null)
                    return Result.Failure(Error.NotFound($"Marka bulunamadi (Id={brand.Id}).", "MaterialBrand.NotFound"));
        ...
        public async Task<Result<bool>> AnyAsync(Expression<Func<MaterialBrand, bool>> predicate, CancellationToken cancellationToken = default)
        {
            ...
            catch (Exception ex)
            {
                ...
                return Result.Failure<bool>(Error.Unexpected("Marka sorgusu yapilamadi.", ex, "MaterialBrand.Any.Failed"));
            }
        }
    }
}